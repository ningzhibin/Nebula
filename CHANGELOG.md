...
### Changed
- **v5.82** (2026-09-03): **One typography system across all pages (tokens + hierarchical scale)** - new single-source-of-truth [css/font_tokens.css](css/font_tokens.css): type stacks (`--font-display` Fraunces, `--font-body` IBM Plex Sans, `--font-mono` IBM Plex Mono), `--fs-root` 16px rem anchor, hierarchical size scale `--fs-2xs` 10 / `--fs-xs` 11 / `--fs-sm` 12 / `--fs-md` 13 / `--fs-lg` 14 / `--fs-base` 15 (body) / `--fs-xl` 16 / `--fs-2xl` 18 / `--fs-3xl` 22 / `--fs-4xl` 28 px, base `body`/`h1–h4` cascade, and `.fs-*`/`.font-*` utilities. Hardcoded font settings removed or mapped onto the scale across: [css/main.css](css/main.css) (~170 `font-size` decls incl. em/rem and the clamp; `:root` token defs removed — tokens now live only in font_tokens.css; 'Courier New'/'Helvetica Neue'/ui-monospace literals → tokens), [css/clustergrammer_bootstrap_restore.css](css/clustergrammer_bootstrap_restore.css) (Segoe UI → `var(--font-body)`, body 16px → `var(--fs-base)` unifying the old 15/16px split, plus scoped `!important` override of the vendored Clustergrammer Helvetica SVG text on `#cgm-container`/`#clustergrammerContainer`), [index.html](index.html) (all inline + JS-string `font-size`/`font-family`; 4 Plotly `family:` literals → `NebulaFonts.body`; 9px SVG micro-labels → 10), all 4 [popout/](popout/) pages (Segoe UI → token family, 12/13px → `--fs-sm/md`, linked `../css/font_tokens.css`), [js/nebula_report.js](js/nebula_report.js) (report CSS now starts with a baked `:root` token block + `code,pre` mono rule; `Segoe UI,Roboto,…` → `var(--font-body)`; the fine rem cascade (0.7–1.35rem) mapped onto the shared `--fs-*` scale; body `15px` → `var(--fs-base)`), [js/hybrid_tab.js](js/hybrid_tab.js) (svgEl gains a `var()` inline-style escape hatch so SVG presentation attrs can carry token refs; 33 SVG family literals → `var(--font-mono/body)`; off-scale SVG sizes 8.5/9.5/10.5/11.5/12.5 → 10/11/12/13; tooltip + legend DOM strings → tokens), [js/saint_embed.js](js/saint_embed.js), [js/table_display.js](js/table_display.js), [js/kmeans_analysis.js](js/kmeans_analysis.js), [js/phospho_normalization.js](js/phospho_normalization.js), [js/batch_effect.js](js/batch_effect.js), [js/data_qc_overall_dashboard.js](js/data_qc_overall_dashboard.js) (3 Plotly `family:` → tokens), [js/scatter_label_optimize.js](js/scatter_label_optimize.js) (canvas measurement default now the app body stack). New [js/nebula_fonts.js](js/nebula_fonts.js): `window.NebulaFonts` (`body`/`mono`/`display`/`css()`/`fs()`) reads the tokens at runtime for JS APIs that need literal font values. Untouched by design (vendored/legacy per js/AGENTS.md): `clustergrammer.js`, `Enrichrgram.js`, `hzome_functions.js`, `send_to_Enrichr.js`, `hybrid_downstream.js`. Bumped `?v=`/`#appVersion` to `5.82`. ([README.md](README.md))
- **v5.81** (2026-09-03): **Table column headers match the button color** - the 8 accent-vermilion header rules (`.table-preview th`, DIANN annotation thead, matrix-check thead incl. `#tablePreview`/`.qc-dt-*`/`#imputationMatrixHost` `!important` variants, `#diffResultsTable th`) moved to `var(--md-btn-primary-bg)` with white text kept; the 4 sortable-header hover rules moved from `--md-accent-hover` to primary-bg plus `brightness(0.92)` filter (same hover language as buttons). Specialty palettes (Hybrid journal, SAINT `#e2e6ef`, Enrichr `#f0f0f0`, PCA `#2c3e50`, batch muted, column-filter lavender) unchanged. Bumped `?v=`/`#appVersion` to `5.81`. ([css/main.css](css/main.css), [README.md](README.md))
- **v5.80** (2026-09-03): **One button font color** - `--md-btn-base-fg` flattened to `#fff` in Warm/Sage/Slate (was `#f6f2ea`/`#f7faf7`/`#f6f8fb`; Contrast already `#fff`), so every solid button renders pure-white text; `.pagination-controls button:hover` now pins `color: var(--md-text-primary)` instead of inheriting white on the light accent-soft fill. Ghost/outline buttons keep dark text. Bumped `?v=`/`#appVersion` to `5.80`. ([css/main.css](css/main.css), [README.md](README.md))
- **v5.79** (2026-09-03): **One button font and height app-wide** - base `button` rule gained explicit `line-height: 1.5` (plus `filter` in the transition for the brightness hover); removed the `.tab-sidebar button` 12px/6px-12px override, the 12px/14px and 8px/10px-20px/`border:none` inlines on ~30 Generate/Run/Cancel/Export/SVG buttons (they now inherit the 13px/7px-14px/1px-border base), and normalized `.heatmap-generate-btn` (10px→7px vertical), `.diff-run-btn` (0.92em→13px, 8px-12px→7px-14px, `border:none`→1px transparent), and `.pagination-controls button` (12px→13px, 6px-12px→7px-14px). Toggle chips (`btn-hist/vol/onest`, hybrid tool buttons, Class I toggle), icon/× buttons (header, modal, delete-column), and Clustergrammer controls keep compact sizes. Bumped `?v=`/`#appVersion` to `5.79`. ([css/main.css](css/main.css), [index.html](index.html), [README.md](README.md))
- **v5.78** (2026-09-03): **One solid button color per theme** - `--md-btn-base-bg` is now `var(--md-btn-primary-bg)` in all four theme blocks (was `var(--md-accent)` in Warm/Contrast, hardcoded accent-equals in Sage/Slate); the muted-gray Clear/Reset inlines (6x), the amber Stop inline, and the accent phospho example loader were converted to `--md-btn-primary-bg`; generic `button:hover/active` moved from `--hyb-vermilion-deep` to primary-bg plus `brightness(0.92/0.85)` filter. Ghost/outline secondary buttons and `.delete-column-btn` danger red unchanged. Bumped `?v=`/`#appVersion` to `5.78`. ([css/main.css](css/main.css), [index.html](index.html), [README.md](README.md))
- **v5.77** (2026-09-03): **Primary CTA buttons follow the color theme** - `#heatmapTab .heatmap-generate-btn`, `#diffTab .diff-run-btn`, and the phospho **Run Normalization** / **Send Normalized Data** inline styles moved from fixed-green `--md-btn-success`/`-hover`/`-active` (same green under every theme) to themed `--md-btn-primary-bg` (matching all other Generate/Run buttons); hover/active feedback is now a theme-agnostic `brightness(0.92/0.85)` filter so no per-theme hover vars are needed. Bumped `?v=`/`#appVersion` to `5.77`. ([css/main.css](css/main.css), [index.html](index.html), [README.md](README.md))
- **v5.76** (2026-09-03): **Buttons follow the color theme** - bare-button base `--md-btn-base-bg` changed from near-black `#211d17` (Warm) / `#000000` (Contrast) to `var(--md-accent)`, so Data Preparation **Load Pasted Data** / **Load Meta Data** and other unclassed buttons render in the theme accent (Sage/Slate already used their accent). Leftover hardcoded buttons converted to theme tokens: Name-mapping **Stop** (`#b45309` → `--md-btn-warn`), Meta Table **Select All Visible** (`#5a67d8` inline removed → cascade), Add-to-Report **Cancel** (`#6b7280` inline removed → ghost `example-btn`), phospho dataview tabs (`#007bff`/`#6c757d`/`#28a745` → `--md-accent`/`--md-btn-muted-bg`/`--md-btn-success` in `showPhosphoDataview`). Bumped `?v=`/`#appVersion` to `5.76`. ([css/main.css](css/main.css), [index.html](index.html), [js/phospho_normalization.js](js/phospho_normalization.js), [README.md](README.md))
- **v5.75** (2026-09-03): **Header color-theme picker (Warm / Sage / Slate / Contrast)** - `Warm` is the current palette; `Sage` (green), `Slate` (blue-gray), and `Contrast` (high-contrast light) re-skin app chrome, tabs, sidebars, tables, status pills, **solid Generate/Run/Apply buttons** (previously hardcoded olive `#6f7f66` / gray `#6c757d` inline, now `--md-btn-primary-bg` / `--md-btn-muted-bg` vars), **bare `<button>` base** (Data Preparation loaders etc., `--md-btn-base-bg/fg`), and **hybrid toggle buttons** (borders/backgrounds plus JS-set active states in `hybrid_tab.js`, now `var(--hyb-line-2)` / `var(--hyb-fig)` / `var(--md-accent)`) through `html[data-theme]` CSS-variable overrides (hardcoded warm washes converted to `rgba(var(--triplet))` vars first; Warm computed values unchanged). Picker lives in the top-right header, persists in `localStorage` (`nebulaTheme`), and a pre-paint head script applies it before first render. Light family only so Plotly/D3 figure canvases stay white-harmonious; no dark theme. Bumped `?v=`/`#appVersion` to `5.75`. ([index.html](index.html), [css/main.css](css/main.css), [README.md](README.md))
- **Unreleased**: **Data PreProcess → Outliers — right-section scrollbar** - the panel's `.visualization-container` now scrolls vertically (`overflow-y: auto`; the app-wide rule is `overflow: hidden`), so the score plot, z-score heatmap, and metrics table are all reachable on short viewports; the table host keeps a capped height (320–520 px) with its own inner scroll. No version bump (trivial fix). ([index.html](index.html))
- **Unreleased**: **Data PreProcess → Outliers — results split into 3 inner tabs** - score plot, z-score heatmap, and metrics table now live in **Score plot | Z-score heatmap | Metrics table** inner tabs (shared `data-filter-inner-tab` style) instead of one stacked page, so each view gets the full panel width; only the active view renders. No version bump (trivial fix). ([index.html](index.html), [doc/outliers.md](doc/outliers.md))
- **v5.74** (2026-09-03): **Data PreProcess → Outliers sub-tab (new)** - robust sample-level outlier detection (missing fraction, median, IQR, skew, mean pairwise Pearson correlation on log10(1+x); MAD modified z-scores with |z| > 3.5 default, ≥2 flags default; score = max |z|). Score bar plot with threshold line, z-score heatmap, sortable metrics table with Export CSV, pre-checked exclusion list applied via `applyColumnSubsetByIndices` with auto re-run, Add to Report (`outlier_score`), Pop out, session/UI-state persistence, and new Document §14 (later sections renumbered 14–24). Bumped `?v=`/`#appVersion` to `5.74`. ([index.html](index.html), [js/nebula_report.js](js/nebula_report.js), [js/figure_popout_buttons.js](js/figure_popout_buttons.js), [doc/outliers.md](doc/outliers.md), [README.md](README.md))
- **v5.73** (2026-09-03): **Clustering → UMAP sub-tab (new)** - full UMAP visualization between t-SNE and K-means, mirroring the t-SNE layout: sidebar with **n_neighbors** (15) / **min_dist** (0.1) / **spread** (1.0) / **epochs** (auto: 500 ≤10k samples else 200) / **seed** (42), same display controls (labels, arrows, 1000×800, title), sampling caps, and preprocessing (log10 + column/row z-score). Compute via **umap-js@1.4.0** CDN (`fitAsync` with epoch progress; seeded RNG for reproducibility), Plotly scatter with group-annotation color/shape, repelled labels, **Add to Report** (`umap` section), **Pop out**, session/UI-state save-restore, and in-app clustering doc. Bumped `?v=`/`#appVersion` to `5.73`. ([index.html](index.html), [js/nebula_report.js](js/nebula_report.js), [js/figure_popout_buttons.js](js/figure_popout_buttons.js), [doc/clustering.md](doc/clustering.md), [README.md](README.md))
- **v5.72** (2026-09-02): **Hybrid Differential → Feature table — rename FDR to Q value** - you are correct, the per-feature `fdr` column is a Storey **q-value** (minimum global FDR at `min p`, the hybrid FDR curve at `min p`, monotonic, capped at 1), not the list-level FDR (stat strips still show the global FDR at `cp*`). Changed `COLS` label `fdr` → `Q value`, updated `COL_HELP` for `fdr`/`pep`, `tableNote` and CSV header `fdr` → `q_value`. Bumped `?v=`/`#appVersion` to `5.72`. ([js/hybrid_tab.js](js/hybrid_tab.js), [index.html](index.html), [README.md](README.md))
- **v5.71** (2026-09-02): **Hybrid Differential → Feature table — cap Feature group column width** - added `max-width:180px; overflow:hidden; text-overflow:ellipsis; white-space:nowrap` to `#hybridTab table.hyb-data th:nth-child(2)` / `td.protein` and `title` hover for the full ID; prevents a few very long group IDs from stretching the table. Bumped `?v=`/`#appVersion` to `5.71`. ([css/main.css](css/main.css), [js/hybrid_tab.js](js/hybrid_tab.js), [index.html](index.html), [README.md](README.md))
- **v5.70** (2026-09-02): **Hybrid Differential — removed Dataset sub-tab** - removed `Dataset` button (`data-hybt="dataset"`) and `hybPanelDataset` panel (`#hybWide*` wide matrix) from `#hybInnerTabs`; `switchHybInner` map now `table/analysis/volcano/hybridScatter/diag` (5 tabs). Nebula's table viewer elsewhere covers this. Bumped `?v=`/`#appVersion` to `5.70`. ([index.html](index.html), [README.md](README.md))
- **v5.69** (2026-09-02): **Hybrid Differential sub-tabs restyled to Differential theme** - replaced custom `9px 16px` / `::after` underline with Differential's `10px / 4px 10px 3px` + `border-bottom:2.5px transparent → var(--hyb-vermilion)` on `.active`; rail now `gap:4px / padding:4px 10px 0 / border-bottom 1.5px` identical to `#diffTab .diff-sub-tabs-row`. Bumped to `5.69`. ([css/main.css](css/main.css), [index.html](index.html))
- **v5.68** (2026-09-02): **Hybrid Differential sticky tabs full-width** - made sticky rail break out of `.hyb-main-inner` padding (`margin:-28px calc(-1*clamp(...))` + `padding:12px clamp(...)`) so no paper container shows around the strip. Bumped to `5.68`. ([css/main.css](css/main.css))
- **v5.67** (2026-09-02): **Hybrid Differential right-panel sub-tabs sticky** - made `#hybridTab .hyb-tabs` sticky (`position: sticky; top: 0; z-index: 5; background: var(--paper)`) so the **Feature table → Diagnostics** strip stays pinned at the top of the scrollable `.hyb-main` (mirrors Differential's sticky pattern). Bumped `?v=`/`#appVersion` to `5.67`. ([css/main.css](css/main.css), [index.html](index.html), [README.md](README.md))
- **v5.66** (2026-09-02): **Hybrid Differential sidebar slimmed; About moved to main panel** - moved the **About Hybrid Differential** card from `.tab-sidebar` to `.hyb-main-inner` (first card above `#hybEmpty`) and then into `#hybEmpty` so it shows only when initiated (hidden when `#hybResults` is visible); sidebar now only **01 · Analysis** after removing **02 · Export** (`#hybBtnCsv/#hybBtnWideCsv/#hybBtnFigPng/#hybBtnVolPng`) and **03 · Reference** — citation stays in the right-panel About/empty-state cards (`— see sidebar 03·Reference` phrase also removed). Bumped `?v=`/`#appVersion` to `5.66`. ([index.html](index.html), [README.md](README.md))
- **v5.65** (2026-09-02): **Downstream → Hybrid Differential moved right after Differential; sidebar streamlined for Nebula-native flow** - sub-tab order now **Differential → Hybrid Differential → SAINT → Enrichr** (`#downstreamSubTabsRow`). Removed the entire **01 · Data source** block (Nebula matrix/Demo/Upload + `hybFileInput`) — Hybrid now auto-reads the current Nebula matrix + meta via `HybridTab.onOpen()`/`buildHybridDatasetFromNebula()` with `Group by`/`Group A/B` moved into **01 · Analysis** (always visible, no `display:none`). Removed the empty-state buttons `#hybQuickNebula`/`#hybQuickDemo` and the manual “Choose data source” steps; replaced with a help card explaining Stage I (presence/absence) + Stage II (intensity, limma/Welch) + Stage III (single hybrid FDR) and reading guide (`H/B/I`, `pep`/`fdr`, one-state). Renumbered sidebar to **01 Analysis → 02 Export → 03 Reference** and expanded Reference with deviations. Bumped `#appVersion`/`?v=` to `5.65`. ([index.html](index.html), [README.md](README.md))
- **v5.64** (2026-09-02): **Hybrid Differential → Hybrid scatter — fixed blank scatter (ID mismatch + cache drift)** - `js/hybrid_tab.js` `drawHybridScatter()` used standalone ids `svgHybridScatter`/`legHybridScatter`/`hybridScatterCards`/`hybridScatterNote` while Nebula's DOM uses `hybFigHybridScatter`/`hybHybridLegend`/`hybScatterCards`/`hybScatterNote` (the `$` shim mapped them but the function bypassed it with `getElementById` and returned early with a blank panel). Patched to `getElementById('hybFigHybridScatter') || getElementById('svgHybridScatter')` (and same for legend/cards/note) and hardened the `figEl`/`legEl` positioning guards; also hardened `exportHybridScatterPng()`. Root cause was the 2026-09-02 12:48 overwrite of `js/hybrid_tab.js` with the standalone `hybrid2/app.js` version (123 kB) over the Nebula-adapted version (125 kB), so testers with the old `?v=5.63` cached file still saw the scatter while fresh loads did not. Bumped `#appVersion` and all `?v=` to `5.64`. ([js/hybrid_tab.js](js/hybrid_tab.js), [index.html](index.html), [README.md](README.md))
- **v5.62** (2026-08-17 21:25): **Data PreProcess → Annotations — the annotation sidebar now adapts to the loaded data format, with a dedicated Class I (unambiguous) site filter for phosphosites and per-column filters in the table headers**: the sub-tab detects the format from `diannMeta` (`source`/`kind` are now also emitted by the DIA-NN parsers - `diann`/`pg`, `diann`/`phospho`, `diann`/`genegroup`; the MaxQuant parser already emitted `maxquant`/`sites|proteinGroups`; header-based inference remains as a fallback) and shows only the matching quick filters: DIA-NN pg keeps the **N.Sequences / N.Proteotypic.Sequences** numeric bounds; MaxQuant proteinGroups gets **Gene names** and **Protein.Group / Protein IDs** keyword fields (the DIA-NN numeric fields are hidden); phosphosites (MaxQuant Phospho (STY)Sites / DIA-NN phosphosites) get a dedicated **Class I (unambiguous) sites only** toggle button (Class I = exactly one token in `Positions within proteins`; DIA-NN phosphosite rows are single localized sites and pass) plus a **Residue** selector (Any/S/T/Y, matched against `Amino acid`/`Residue`). The generic all-columns keyword filter (Contains/Regex) remains, and every annotation table column now has a **live filter input inside its header** (DataTables per-column search, debounced ~150ms; values persist in `annHeaderFilters` and are part of Apply). Apply now runs through the new generic predicate `rowPassesAnnotationFilter()` + `collectAnnotationSidebarCriteria()` (the old DIANN-only `rowPassesDiannSidebarAnnotationFilter()` was removed) and still commits via the shared pipeline (`materializeRowIndexSplit`/`commitRowFilterToState`; source id stays `diannAnnotationSidebar`, so existing snapshots/undo keep working); **Clear criteria** also resets the header filters, and they reset on new data load. Files: [index.html](index.html) (sidebar HTML, adaptive logic, header-filter wiring in `updateDiannAnnotationTablePreview`), [css/main.css](css/main.css) (`.ann-col-filter*`, `.ann-classi-btn`), [doc/diann.md](doc/diann.md) + [doc/docs_bundle.js](doc/docs_bundle.js) (rebuilt).
- **v5.61** (2026-08-17 19:53): **Data Preparation → Phospho/total paired → Normalization — the Sequential KNN block is now fully aligned with the Imputation sub-tab's SeqKNN (shared worker + shared parameter set)**: the phospho tab no longer uses its own standalone `imputeKNN()` (removed from [js/phospho_normalization.js](js/phospho_normalization.js), along with `_phColumnMedians`); **Run Normalization** now routes both the phosphosite and proteome matrices through the **same SeqKNN worker** the Imputation sub-tab uses (`runImputationKnnAsync()` from [js/missing_value_imputation.js](js/missing_value_imputation.js) → [js/imputation.worker.js](js/imputation.worker.js)), one after the other (the worker is a singleton) with per-matrix progress in the status line. The **Imputation (Sequential KNN)** settings block in [index.html](index.html) now mirrors the Imputation sub-tab's SeqKNN controls with the same defaults: **K neighbors** (5; range now 1-100 as there), **Distance metric** (Euclidean/Manhattan/Pearson/Cosine), **Weighting** (Inverse distance/Uniform/Rank-based), **Min. shared columns** (1), **Fallback when no donor value** (Column median/Column mean/Global minimum/Zero), **Neighbor axis** (Features-rows/Samples-columns), and **Max missing % per row (skip above)** (default **50**, same as the sub-tab). The per-group cap is intentionally absent: the phospho workspace's meta table is per-feature annotation (protein/gene/site rows, used for protein matching), not per-sample metadata, so there are no sample groups to cap against — the worker's per-group cap stays at its "disabled" default (no `columnGroups` sent). `buildPhosphoKnnOpts()` collects the settings into the worker's exact `knnOpts` shape; `runPhosphoKnnImpute()` wraps the shared async runner with status-line progress. `zeroIsMissing` is false because both matrices are log2-transformed first (values ≤ 0 are already null). Doc *3. Data input* updated. ([js/phospho_normalization.js](js/phospho_normalization.js), [index.html](index.html), [doc/input.md](doc/input.md), [doc/docs_bundle.js](doc/docs_bundle.js), [README.md](README.md))
- **v5.60** (2026-08-17 19:06): **Data Preparation → Phospho/total paired → Normalization — the standalone KNN imputer is now Sequential KNN (SeqKNN), aligned with the main imputation KNN**: `imputeKNN()` in [js/phospho_normalization.js](js/phospho_normalization.js) was rewritten from a simple simultaneous KNN (every row imputed independently from a frozen copy, plain neighbor mean, zero fallback) to **Sequential KNN** matching the Data PreProcess → Imputation algorithm ([js/imputation.worker.js](js/imputation.worker.js)): rows are imputed **fewest-missing first** and each imputed row is **promoted into the donor pool** so later, more-missing rows can borrow from already-filled rows (distances read from the progressively-filled matrix); missing cells are the **inverse-distance-weighted mean** of the K nearest donors (Euclidean distance over co-observed samples, normalized by shared-column count), with a **per-column median fallback** (new `_phColumnMedians` helper) instead of 0 when no donor has a value. This also fixes the old code's mismatch between its "distance-weighted average" comment and its actual unweighted mean. UI/labels updated: the sidebar block is now **Imputation (Sequential KNN)** / **Enable Sequential KNN imputation** (with a tooltip describing the algorithm), and the status line reads *"… Sequential KNN Impute …"*. Doc *3. Data input* updated. ([js/phospho_normalization.js](js/phospho_normalization.js), [index.html](index.html), [doc/input.md](doc/input.md), [doc/docs_bundle.js](doc/docs_bundle.js), [README.md](README.md))
- **v5.59** (2026-08-17 17:32): **Differential → Results table — in-cell graph is now a line chart; chart-type options moved to the detail plot**: the **Quant** in-cell column now always draws a compact **per-sample line chart** (a sparkline connecting each sample's value in group order, with dots colored by group) — the previous per-cell Boxplot/Violin/Off selector was removed. The chart-type choice now lives with the **detail plot below the table** as a new **Detail plot: Line / Bar / Boxplot / Violin** selector (`#diffQuantDetailMode`, default **Boxplot**): **Line** and **Bar** show every sample (points/bars colored by group, with per-group labels + n and dashed group separators), while **Boxplot** and **Violin** summarize each group. Changing the selector re-renders the currently selected row's plot (tracked via `diffQuantSelectedRow`); clicking a different row keeps the chosen chart type. `diffQuantValuesForFeature` now also returns each group's `raw` per-sample values (sample order, missing→null) for the line/bar modes; `diffQuantMiniSvg` is line-only; `diffQuantDetailSvg` gained `line`/`bar` branches (shared axis via a `wrapSvg` helper) alongside the existing `box`/`violin`. ([index.html](index.html), [doc/differential.md](doc/differential.md), [doc/docs_bundle.js](doc/docs_bundle.js), [README.md](README.md))
- **v5.58** (2026-08-17 15:36): **Differential → Results table — per-sample quantification columns + in-cell box/violin mini-graph + a detail plot below the table**: the results table (rendered via `TableDisplay.renderGenericTable`) now carries the feature's actual quantification, not just the summary stats. (1) A new **Quant** column (right after *Feature*) draws a tiny inline-SVG per-group distribution for each row — **Boxplot** (quartiles + whiskers + median) or **Violin** (Gaussian-KDE, Silverman bandwidth) — colored by group (two groups for a two-group test, all k groups for ANOVA/Kruskal). (2) A **"Show sample quantification columns"** checkbox (default on) appends one numeric column per sample (grouped by condition, values on the tested scale — log2 if the analysis used log2, else raw — with the standard column-scaled bar cells). (3) An **In-cell graph: Boxplot / Violin / Off** selector toggles the mini-graph. (4) **Clicking any row** renders a larger per-group box/violin plot (axis ticks, group labels + n, jittered points) in a new **`#diffQuantDetail`** area **below the table**, and highlights the selected row; the first visible row is shown automatically. Values are pulled live from the working matrix using per-group sample-column indices now stored on the result (`quantGroups` on both the two-group and ANOVA `diffAnalysisLastResult`), honoring the analysis's log2 / *treat-0-as-missing* settings. New helpers: `diffQuantValuesForFeature`, `diffQuantBoxStats`, `diffQuantKde`, `diffQuantMiniSvg`, `diffQuantDetailSvg`, `drawDiffQuantDetail`. ([index.html](index.html), [css/main.css](css/main.css), [README.md](README.md))
- **v5.57** (2026-08-16 20:57): **Data PreProcess → Imputation — new "Seq KNN then MinProb (MCAR + MNAR hybrid)" method**: a new `imputationMethod` option `knnMinProb` that runs a two-stage imputation. **Stage 1**: Sequential KNN with the existing per-row / per-group missing-% caps, so only rows *within* the caps are filled from neighbors (treated as MCAR). **Stage 2**: `imputeMinProb` fills every cell that Stage 1 left missing — i.e. the rows above the caps — with left-censored (MNAR) draws. MinProb's per-column mean/SD are estimated from the **original observed** values (the KNN worker imputes a structured-clone copy, so the source `matrix` is untouched when the MinProb step runs; cells are combined as *KNN value if filled, else MinProb value* via the new `_isMissingValue` helper). Selecting the hybrid shows **both** the Sequential KNN and MinProb parameter blocks plus a one-line explainer (`#imputationHybridHint`); `onImputationMethodChange()` now toggles blocks by `usesKnn`/`usesMinProb`, the status line shows *"Step 1/2 …"* / *"Step 2/2 - MinProb on remaining missing…"* and a summary of how many rows went to each stage, and `_imputationMethodLabel('knnMinProb')` → "Seq KNN + MinProb". Doc *15. Missing value imputation* updated. ([index.html](index.html), [js/missing_value_imputation.js](js/missing_value_imputation.js), [doc/imputation.md](doc/imputation.md), [doc/docs_bundle.js](doc/docs_bundle.js), [README.md](README.md))
- **v5.56** (2026-08-16 20:46): **Data PreProcess → Imputation → Sequential KNN — added a per-group missingness cap; both caps now default to 50%**: new controls in the SeqKNN parameter block — a **Group by (meta column)** dropdown (`#imputationKnnGroupCol`, populated from the meta headers, `(none)` disables it) and **Max missing % per group per row (skip above)** (`#imputationKnnMaxMissingGroup`, default **50**). A row is now left un-imputed if its **overall** missing % exceeds the row cap **or** if **any group's within-group** missing % exceeds the per-group cap — so features that are largely absent in a whole group aren't imputed across groups (which would fabricate group differences). The per-group cap only applies when neighbor axis = **Features (rows)** and a group column is chosen; grouping reuses the same sample-id resolution / replicate fallback as the **Group completeness** filter (new globals `buildColumnGroupKeysForImputation()` and `refreshImputationKnnGroupOptions()`, the latter hooked into `onImputationMethodChange()` so the dropdown refreshes on tab open / method switch). The **default of the existing "Max missing % per row" cap was changed from 100 to 50** per request, so out of the box both caps skip rows above 50% missing. Worker builds a group→column-index map (only when the working matrix keeps its sample-column orientation, i.e. not transposed) and enforces both caps in the split step; `columnGroups` / `maxMissingFracPerGroup` are passed via `knnOpts` and recorded in `imputationAppliedParams`. Doc *15. Missing value imputation* updated. ([index.html](index.html), [js/imputation.worker.js](js/imputation.worker.js), [js/missing_value_imputation.js](js/missing_value_imputation.js), [doc/imputation.md](doc/imputation.md), [doc/docs_bundle.js](doc/docs_bundle.js), [README.md](README.md))
- **v5.55** (2026-08-16 20:29): **Data PreProcess → Imputation → Sequential KNN — new "Max missing % per row (skip above)" cap**: added a per-row missingness threshold (the `rowmax` concept from R's `impute.knn` / SeqKNN). New sidebar control **`#imputationKnnMaxMissing`** (default **100** = impute every row, no cap). Rows whose missing percentage — measured over the neighbor axis (features by default, samples when axis = columns) — **exceeds** the cap are **left un-imputed**: their missing cells stay missing and the row is **never added to the donor pool**, so very sparse rows are not filled from one or two observed values. Wired through `runImputation()` as `knnOpts.maxMissingFrac` (pct/100) and enforced in the worker (`missThreshold = maxMissingFrac * cols`; rows with `miss > threshold` are counted as `skippedRows` and excluded from both the impute list and the donor pool); the worker returns `skippedRows`, surfaced via `imputationLastSkippedRows` in the success status (e.g. *"… 1210 cells -> 40 missing. (3 row(s) left un-imputed: over the max missing % cap)"*) and recorded in `imputationAppliedParams.maxMissingPct`. Doc *15. Missing value imputation* updated. ([index.html](index.html), [js/imputation.worker.js](js/imputation.worker.js), [js/missing_value_imputation.js](js/missing_value_imputation.js), [doc/imputation.md](doc/imputation.md), [doc/docs_bundle.js](doc/docs_bundle.js), [README.md](README.md))
- **v5.54** (2026-08-16 20:18): **Data PreProcess → Imputation — the KNN method is now Sequential KNN (SeqKNN)**: the nearest-neighbor imputer in [js/imputation.worker.js](js/imputation.worker.js) was replaced with **Sequential KNN (SeqKNN; Kim, Golub & Kim, *Bioinformatics* 2004)**. Instead of imputing every row independently from a frozen copy of the original matrix, rows are now imputed **one at a time, ordered by ascending missing count (fewest-missing first)**; each row is imputed from the current pool of *complete* donor rows and then **promoted into that pool**, so later, more-missing rows can borrow from already-filled rows (distances and donor values are read from the progressively-filled matrix). Rows encountered before the complete-pool has any members fall back to the per-column fallback value and then join the pool. All existing options are preserved (K, distance metric, weighting, min. shared columns, fallback, neighbor axis). UI renamed accordingly: the method dropdown option is now **Sequential KNN (SeqKNN, MCAR)**, the sidebar section header is **Sequential KNN parameters** (with a tooltip explaining the algorithm), and the status/progress line reads *"Sequential KNN: N / M rows imputed"* / *"Imputation applied (Sequential KNN)."* (`_imputationMethodLabel` maps internal method ids to friendly labels). Doc *15. Missing value imputation* updated (and the earlier v5.53 default swap to MinProb is now reflected there too). ([js/imputation.worker.js](js/imputation.worker.js), [js/missing_value_imputation.js](js/missing_value_imputation.js), [index.html](index.html), [doc/imputation.md](doc/imputation.md), [doc/docs_bundle.js](doc/docs_bundle.js), [README.md](README.md))
- **v5.53**: **Data PreProcess → Imputation — MinProb is now the default method + a "not recommended" warning banner**: the `#imputationMethod` dropdown's selected option moved back from **KNN** to **MinProb (MNAR — left-censored)** ([index.html](index.html)), so the sidebar opens with **MinProb parameters** (Down-shift 1.8 / Width 0.3) visible and KNN parameters collapsed. A dismissible-styled amber **warning banner** now sits at the very top of the Imputation sidebar (above the *Missing Value Imputation* heading): *"Data imputation is not recommended, missingness itself is informative, and imputation under a mismatched mechanism inflates type I error."* ([index.html](index.html))
- **v5.53**: **Data PreProcess → Imputation — MinProb is now the default method + a "not recommended" warning banner**: the `#imputationMethod` dropdown's selected option moved back from **KNN** to **MinProb (MNAR — left-censored)** ([index.html](index.html)), so the sidebar opens with **MinProb parameters** (Down-shift 1.8 / Width 0.3) visible and KNN parameters collapsed. A dismissible-styled amber **warning banner** now sits at the very top of the Imputation sidebar (above the *Missing Value Imputation* heading): *"Data imputation is not recommended, missingness itself is informative, and imputation under a mismatched mechanism inflates type I error."* ([index.html](index.html))
- **v5.52**: **Differential analysis — limma (moderated t) is now the default two-group test**: the **Test (two groups)** dropdown defaults to **Moderated t (limma)** (was Welch). limma's empirical-Bayes variance moderation (Smyth 2004) shrinks each feature's variance toward a prior — more stable with small sample counts and missing values, the standard in proteomics differential expression. Also fixed the per-feature `stdev.unscaled` so each feature uses its own non-missing counts per group (previously the full group sizes were used for every feature, underestimating SE for proteins with missing values), and the in-app doc now reflects the limma default. The **Comparison mode** option was relabeled to **Two groups** (the test is chosen in **Test (two groups)**) and **Run differential analysis** moved to the bottom of the sidebar. ([index.html](index.html), [js/moderated_t.js](js/moderated_t.js), [css/main.css](css/main.css), [doc/differential.md](doc/differential.md))
- **v5.51**: **Quick Report pipeline gets a progress bar and runs in the background** - clicking **Generate Quick Report** now shows a progress bar at the top of the Report tab's right side (stage label + percentage + detail line; the heatmap step tracks the heatmap worker's real progress via `#heatmap-prog-pct`, PCA interpolates within its 90s budget) instead of only the small sidebar status text. The run is fully non-blocking: heatmap/PCA keep running in their Web Workers with async polling, the floating global progress overlay is suppressed (redirected into the in-tab bar) so it never pops over other tabs, clicking the button again while a run is active shows "Pipeline already running", and the pipeline only switches you to the Report tab if you never navigated away mid-run (otherwise it finishes quietly in the background and the preview is ready when you return). ([index.html](index.html), [css/main.css](css/main.css))
- **v5.50**: **Versioning policy — the header version now moves with notable updates** - every change that earns a README bullet now also bumps the `#appVersion` span (top-right header) and every `?v=` cache-buster in `index.html` in the same pass (rule documented in `AGENTS.md`). This batch syncs the app from v5.39 to **v5.50** and unifies the drifted cache-busters (5.11/5.39/5.40/5.43 -> 5.50). ([index.html](index.html), [AGENTS.md](AGENTS.md))
- **v5.50**: **Tab hierarchy now shrinks level by level** - the third-level inner tabs (`Table | Sankey` under Meta Table, Imputation/Batch Effect tabs, Data Filter Row/Column, Differential/SAINT/Enrichr sub-tabs, Data QC chart tabs, ...) go from 11px/`6px 11px 5px` to **10.5px/`4px 10px 3px`** and the fourth-level Enrichr plot tabs to **10px/`3px 9px 2px`**, so the visual hierarchy is now clearly L1 (39px) > L2 (33px) > L3 (22px) > L4 (20px) - each deeper level is visibly smaller than the one above it. ([css/main.css](css/main.css))
- **v5.49**: **Compact table rows app-wide** - body rows of the shared DataTables views (Data Filter Passed, Data Preparation previews, Imputation Result Data) shrink from ~38px to **26-28px** (CDN td padding overridden to `4px 10px` + `line-height: 1.35`; bar cells slimmed from 20px to 18px, in-cell intensity bars still render with their fill); the Meta table and DIANN annotation preview cells drop from 8px to **5px 8px** padding (rows ~28px), and the Differential results cells go to 5px 8px. Header/row heights now share one compact rhythm. ([css/main.css](css/main.css))
- **v5.48**: **Compact table headers app-wide** - the shared DataTables views (Data Filter Passed, Data Preparation previews, Imputation Result Data) override the DataTables CDN header padding (`10px 30px 10px 10px`, ~38px tall) with a compact `5px 24px 5px 8px` + `line-height: 1.3` (~28px; sorting arrows still fit and sorting works); the Meta table preview headers shrink the same way (the delete button's stray 6px top margin that inflated the header row was removed; header rename/edit unaffected); `.table-preview` and DIANN annotation headers use `6px 8px`. All table headers now share one compact height matching the 12px table font. ([css/main.css](css/main.css))
- **v5.47**: **Document tab — full-width content + optional reading width** — the help content now fills the whole page (the 1200px caps on `.doc-markdown-main` / `.doc-prose` are gone), and the Document sidebar gained a **Reading** section with a **Limit text width (easier reading)** checkbox: checked = prose capped at 960px and centered, unchecked = full page width. The choice persists in `localStorage` (`nebulaDocLimitWidth`) and survives section switches and page reloads. ([index.html](index.html), [js/doc_viewer.js](js/doc_viewer.js), [css/main.css](css/main.css))
- **v5.46**: **Documentation (in-app help) rewritten as a 22-section English user manual** - the Document tab (header **?** button) ships a complete, rewritten manual covering every feature, replacing the old changelog-style notes: Overview & navigation, Data Preparation (formats, example data, phospho paired analysis, session snapshots), DIA-NN / MaxQuant upload rules, Name mapping, Replicate parsing, Data Filter (row/column pipeline), Group completeness, Post-filter behavior, Imputation, Batch Effect, Meta table & Sankey, Data QC (Overall, Row profile, Column profile, Column correlation, UpSet/Venn/K-map), Clustering (Heatmap, Clustergrammer, PCA, PCoA, t-SNE, K-means), Differential, SAINT, Enrichr, and Report export. `doc/manifest.json` renumbered 1-22; the Document outline now mirrors the app's tab structure (group headers **About Nebula / Data preparation / Data QC / Data preprocess / Clustering / Downstream / Report** with indented section buttons, matching each main tab one-to-one); `doc/docs_bundle.js` regenerated. ([doc/*.md](doc/), [js/doc_viewer.js](js/doc_viewer.js), [css/main.css](css/main.css), [README.md](README.md))
- **Data PreProcess → Meta Table → Sankey inner tab**: the Meta Table sub-tab now has two inner tabs (**Table | Sankey**, `switchMetaTableInnerTab('table'|'sankey')` toggling `#metaTableInnerTablePanel` / `#metaTableInnerSankeyPanel` — the old standalone "Meta Sankey" sub-tab was folded in). The **Sankey** tab visualizes the meta table's hierarchical structure as an interactive **Plotly Sankey** diagram — each selected meta column is one level, each sample row is a flow through the columns' values, and link widths show how many samples share each value-to-value transition. The sidebar checklist (`#metaSankeyColumnList`) toggles included columns and reorders them by **drag-and-drop** (checked rows are `draggable`; dragover shows an insert-before/after indicator line, dropping onto the unchecked tail or the empty space below appends to the end of the hierarchy, and the container-level drop handlers are bound once so list rebuilds don't pile up listeners) or with ↑/↓ buttons (`buildMetaSankeyColumnList`); a **Max nodes per level** field (`#metaSankeyMaxNodes`, default 30) merges rarer values into an "(other N)" node per level; rows missing a value in any selected column are skipped and counted in the status line (`#metaSankeyStatus`). `drawMetaSankey()` builds nodes/links from `metaData` and renders via `Plotly.react` (no new dependency); the figure is drawn only while the inner tab is visible (explicit pixel size measured at draw time, avoiding the hidden-panel Plotly fallback-size lock) and re-fits whenever the Sankey inner tab is (re)activated. `updateMetaTablePreview()` refreshes it whenever the meta table changes, and `switchTableSubTab('metaTable')` re-applies the remembered inner tab on open. **Vertically centered levels**: each level's node group is centered in the plot height instead of bottom-aligned (Plotly's default `snap` arrangement bottom-anchors short columns) — `drawMetaSankey()` renders in two passes: pass 1 draws with empty `node.x`/`node.y` so Plotly/d3 computes the real per-node heights, then reads them back from `gd._fullData[0]._sankey.graph.nodes` (grouping by `x0`, ordering by `y0`) and pass 2 re-renders with `arrangement: 'perpendicular'` and explicit `node.y` centers (fractions of `_sankey.dragParallel`, epsilon-clamped to `[1e-6, 1-1e-6]`); each column is centered on its **measured rendered span** (`first.y0`..`last.y1`), not a pad-derived total — d3 sankey shrinks `node.pad` to fit the figure, and a pad-based total overshoots the height, pinning the last leaf past the bottom edge and leaving the whole column off-center on cold renders; the first column's nodes get a tiny truthy `x` (`1e-4`) because Plotly skips any node whose `x` or `y` is falsy. **Draggable nodes**: `'perpendicular'` makes every node draggable vertically within its level (x stays locked to the column; Plotly's `'snap'` drag path was rejected because it runs through a force loop that stalls on dense columns and leaks a drag-cover that blocks all later mouse interactions) — drag-end persists the new center into `node.y` (`persistFinalNodePositions` divides by `dragParallel`, the same fraction scale as the centering), so dragged positions survive redraws (`drawMetaSankey` reuses the previously-drawn centers as `prevValid` instead of re-centering) and column toggles. ([index.html](index.html), [css/main.css](css/main.css), [README.md](README.md))
- **Data Preparation — new third column previews the loaded matrix & meta table**: the Data Preparation tab now has a third column (`data-prep-col-right`) beside the two upload panels, holding two live sub-tables — **Loaded data matrix** and **Loaded meta table** — rendered via the shared `TableDisplay` pipeline (`renderMatrixPreview` with row-scale numeric bars for the matrix, `renderGenericTable` for the meta table using safe column keys so headers containing dots/spaces map correctly). `updateDataPrepMatrixView()` / `updateDataPrepMetaView()` are hooked into `updateTablePreview()` / `updateMetaTablePreview()` (both the no-data and success paths), so the previews refresh whenever a matrix is loaded/pasted/uploaded (including DIA-NN/MaxQuant/SAINT parsers and session restore) or the meta table is generated/edited/imported; `switchTab('dataPrep')` re-renders both on tab activation so DataTables always measures a visible container (hidden panels measure 0), and boot-time init calls show the empty-state placeholders on first load. ([index.html](index.html), [css/main.css](css/main.css), [README.md](README.md))
- **Help text converted to tooltips across sidebars and panels**: every static helper paragraph is gone from the UI. (1) **Sidebar intros** under **Missing Value Imputation**, **Batch Effect**, **Overall** (Data QC), **Column profile**, **Column Correlation**, **SAINT**, **Meta Table**, and **Document** are now a **"?" badge** (`.sidebar-help-icon` in [css/main.css](css/main.css)) with the same wording in a native `title` tooltip on the `h2` heading. (2) The eight `heatmap-field-hint` paragraphs were converted to `title` tooltips on their controls: the **Bar metric** label (Overall, badge + title), the **Notes** heading (Overall, merged into its existing tooltip — the section now holds just the heading), `#batchEffectBatchCol`, `#maxHeatmapFeatures`, the **Log10 transform** label (badge + title), `#heatmapFeatureLabelSelect` (merged into its existing title), `#heatmapMarginBottom` (merged into its existing title), and the **Sample group overlays** heading (badge + title). (3) The redundant **Column Select** intro paragraph was deleted (its `h3` already carries the same guidance in a tooltip). All eight `<p class="...field-hint">` paragraphs and the unused `#heatmapTab .heatmap-field-hint` CSS rule were removed; roughly 30 lines of static text saved app-wide. ([index.html](index.html), [css/main.css](css/main.css), [README.md](README.md))
- **Data PreProcess → Imputation — KNN is now the default method**: the `#imputationMethod` dropdown's selected option moved from **minProb** to **knn** (`<option value="knn" selected>KNN (MCAR — distance-weighted)</option>` in [index.html](index.html)); the `'minProb'` fallbacks in `onImputationMethodChange()` and `runImputation()` became `'knn'` ([js/missing_value_imputation.js](js/missing_value_imputation.js)). Since the startup `onImputationMethodChange()` call in [index.html](index.html) syncs the parameter blocks, the sidebar now opens with **KNN parameters** (K neighbors, default 5) visible and **MinProb parameters** collapsed until MinProb is picked. ([README.md](README.md))
- **Data PreProcess → Imputation — new "Matrix" inner tab (full DataTables matrix view)**: the Imputation inner tab row (now **Overview · Distribution · All samples · Matrix**) added **Matrix** — the full intensity matrix rendered via the shared `TableDisplay.renderMatrixPreview` pipeline (the same DataTables view as **Data Filter → Passed**: search, sort, paging, frozen sticky header, dual-axis scroll shell, and bar-style numeric cells with the indigo bar fill, mirrored from the `#tablePreview` rules under `#imputationMatrixHost`). `switchImputationInnerTab('matrix')` toggles `#imputationMatrixPanel` and refreshes on activation; `finish()`/`resetImputation()` refresh it alongside the other figures; `refreshImputationMatrix()` skips rendering while the panel is hidden (the DataTables scroll shell measures the panel at draw time — the tab re-renders on open) and shows the standard "No data to display." empty state when no matrix is loaded. The Data PreProcess sub-tab activation block also refreshes it on re-open. **Imputed-cell highlighting**: `finish()` snapshots a per-cell missing mask (`_missingMaskOf`, same missing definition as the stats — null/NaN, plus zero when **Treat 0 as missing** is on) before overwriting and stores it in `imputationMask` (nulled by `resetImputation()`); `refreshImputationMatrix()` passes it as `highlightMask` to `renderMatrixPreview`, whose numeric-cell renderer flags `qc-dt-imputed-cell` on the matching `_origRowIndex` rows — green-tinted bar cells with a bolded value and an "Imputed value (this cell was missing before imputation)." tooltip — and toggles a one-line legend (`#imputationMatrixHint`) above the table. The mask is captured from the pre-imputation matrix, so with the default non-repetitive Apply it always marks exactly the cells the last Apply filled. ([js/missing_value_imputation.js](js/missing_value_imputation.js), [index.html](index.html), [css/main.css](css/main.css), [README.md](README.md))
- **Data PreProcess — active-transformation indicator strip removed**: the full-width **`#tableTransformsIndicator`** strip that sat between the Data PreProcess sub-tab row and the panel content — showing e.g. *"Imputed (MinProb) / Batch-corrected (comBat)"* after imputation/batch correction — is removed. Its markup ([index.html](index.html)), the `#tableTransformsIndicator` CSS rule ([css/main.css](css/main.css)), and the `updateTransformsIndicator()` function plus its call sites in `finish()`/`resetImputation()` ([js/missing_value_imputation.js](js/missing_value_imputation.js)) and the batch-effect apply/reset paths ([js/batch_effect.js](js/batch_effect.js)) are deleted; the per-tab sidebar status lines (`#imputationStatus`, batch-effect status) remain the single source of transformation state. ([README.md](README.md))
- **Data PreProcess → Imputation — Distribution tab now has a per-sample scope (Samples tab merged in)**: the separate **Samples** inner tab was merged into **Distribution**, so the Imputation inner tab row is now **Overview · Distribution · All samples**. The Distribution panel leads with a **Sample** dropdown (`#imputationDistributionScope`, populated from `currentData.columnHeaders` with **"All samples (combined)" as the default** option), and `refreshImputationDistribution()` renders either the original **pooled** before/after KDE overlay across the whole matrix (title *"Log10 intensity distribution (before vs after)"*) or the **single-sample** overlay (title *"Log10 intensity distribution — s20"*) for the chosen column — both share the exact same conditional trace logic as before (single purple "Before imputation" curve pre-imputation; purple + green "After imputation" overlay once an imputation runs; legend always shown; message instead of an empty plot when a sample has <2 usable values). The chosen scope survives redraws, tab switches, and the `finish()`/`resetImputation()` refreshes. The Samples tab button, `#imputationSamplesPanel`/`#imputationSamplesHost` markup, `refreshImputationSamples()` and its CSS were removed. ([js/missing_value_imputation.js](js/missing_value_imputation.js), [index.html](index.html), [css/main.css](css/main.css), [README.md](README.md))
- **Data PreProcess → Imputation — new "All samples" inner tab (single ridgeline plot)**: the Imputation inner tab row (now **Overview · Distribution · All samples**) added **All samples** — a single ridgeline plot stacking **every sample column** — one row per sample, each row a **Gaussian KDE density curve** of that sample's log10(intensity) (same Silverman-bandwidth `_computeKdeCurve`/`_subsampleForKde` pipeline as Distribution), **peak-normalized to 0.85 of the row unit** so ridges read like the classic ridgeline/joyplot style, with the row's **sample name as the y tick label**. Rows are 46px tall, so the plot height scales with sample count (`min(70 + cols·46, 2200)`px) and the panel scrolls for wide matrices. Before/after imputation follows the same conditional logic as the other tabs: before any imputation each row shows a single purple **"Before imputation"** ridge; after an imputation runs, each row overlays the purple **"Before imputation"** + green **"After imputation"** ridges (same `#8884d8`/`#82ca9d` pair), the title gains *" — before vs after imputation"*, and the legend shows exactly one entry per series (each ridge is a `fill: 'toself'` polygon closed at its own row baseline — no baseline traces — with `legendgroup` + `showlegend` only on the first trace of each group). `switchImputationInnerTab('allsamples')` toggles `#imputationAllSamplesPanel` and refreshes on activation, and `finish()`/`resetImputation()` refresh it alongside the other figures. ([js/missing_value_imputation.js](js/missing_value_imputation.js), [index.html](index.html), [css/main.css](css/main.css), [README.md](README.md))
- **Data PreProcess → Imputation — Apply never compounds imputations by default**: a new **Repetitive imputation** checkbox in the sidebar's Options section (**unchecked by default**). With it unchecked (default), **each Apply imputation restores the original matrix first and imputes from it**, so clicking Apply repeatedly never re-imputes the already-imputed matrix (previously every click compounded the imputation and overwrote the saved original with the imputed matrix, breaking **Reset to original**). When **checked**, Apply keeps the old behavior and imputes the current matrix as-is (for repeated/pipeline-style runs). The true original snapshot is now kept across applies — `finish()` only re-snapshots it on the first apply or when the matrix dimensions change (e.g. new data loaded) — so **Reset to original always restores the source data**, and the "Missing before" count in the status line reflects the original matrix on every non-repetitive apply. ([js/missing_value_imputation.js](js/missing_value_imputation.js), [index.html](index.html), [README.md](README.md))
- **Data PreProcess → Imputation — Distribution plot is now overlaid density curves**: the before/after log10-intensity comparison in the Distribution inner tab is no longer a mirrored violin pair — it is a **distplot-style overlay of two Gaussian KDE curves** (Before = purple, After = green), both rising from a common zero baseline with **translucent fills** so the overlap region blends and the shape shift after imputation reads directly (the after-curve's mass visibly shifts right, i.e. higher intensities). The KDE uses a **Silverman bandwidth** over a shared 160-point grid, computed client-side (`_computeKdeCurve`, `_subsampleForKde` caps large matrices at 4000 sampled values so refresh stays fast), and the **figure fills the full height of the panel** (flex-stretched host, re-measured responsively, `responsive: true`). The **numeric y-axis ("Density") auto-ranges to the tallest peak**, so curves are never clipped — the previous categorical-axis approach (violins + `autoFitImputationViolinRange` range hack hooked to `plotly_afterplot`) was removed entirely; x remains `log10(intensity)`. **The legend is always shown** (`showlegend: true`), so before any imputation the single current-data curve is explicitly labeled **"Before imputation"** (purple); once an imputation runs, the purple "Before imputation" and green "After imputation" curves appear together. ([js/missing_value_imputation.js](js/missing_value_imputation.js), [README.md](README.md))
- **Data QC → Overall — per-column summary y-axis now in matrix column order (top-to-bottom)**: the per-column summary bar chart previously listed the first sample column at the **bottom** of the y-axis (Plotly's default for categorical axes); it now sets `yaxis.autorange: 'reversed'`, so samples read **top-to-bottom exactly in the loaded matrix's column order** — the same orientation as the **Data PreProcess → Imputation** overview bar chart. ([js/data_qc_overall_dashboard.js](js/data_qc_overall_dashboard.js), [README.md](README.md))
- **Data PreProcess → Imputation — overview plot is now a horizontal before/after paired bar chart + MinProb fix**: the overview figure (which showed per-column missing counts) is no longer a heatmap — it is a **horizontal grouped bar chart** of **missing % per column** with **two bars per sample — purple "Before imputation" vs green "After imputation"** (the same color pair as the Distribution inner tab's overlaid density curves; **before any imputation is applied only the purple "Before imputation" bars are drawn** — the After series appears only once an imputation has actually been run), **slim bars** (`width: 0.45`, `barmode: 'group'`, matching **Data QC → Overall → per-column summary**), and its **height now scales with the sample count** using the same `overallDashHeightForSamples` helper as **Data QC → Overall → per-column summary** (base 88px + 20px per column, clamped 260–2200px), so **every column label fits on the y-axis** with bars ordered **top-to-bottom in matrix column order**. Since every imputation method fills all missing cells, the green after-bars are ~0% on a fully-imputed matrix — they are drawn as a **minimum sliver** (`max(0.2, 3% of the tallest before-bar)`), so the paired series stays visible, while **hover still reports the true missing %** (via `customdata`). Also fixed the underlying data bug (the old heatmap's z matrix was transposed, 1×N instead of N×1), moved the **summary text (Matrix / Imputation method / Missing before / Missing after) from above the plot into the bottom of the sidebar** (above the status line), and pinned the **status message to the bottom of the sidebar** (`#imputationStatus`, `margin-top: auto`) instead of sitting between the method controls and the Apply button. **MinProb no-op fix**: the downshifted-normal imputation computed its mean/SD/shift on **raw intensities**, so on typical raw-scale data (mean ~1e8, SD ~6e8) the shift went deeply negative and every sampled value clamped to `max(0, …)` = 0 — the matrix came back byte-identical and the status read "1210 cells → 1210 missing". It now computes in **log10(1+x) space** and back-transforms (`10^x − 1`), the scale MinProb is defined on (Perseus/MaxQuant-style MNAR imputation), so imputed values are realistic down-shifted intensities that actually replace the missing cells. ([js/missing_value_imputation.js](js/missing_value_imputation.js), [index.html](index.html), [README.md](README.md))
- **Add to Report on the remaining Data QC figures**: the **UpSet / Venn / K-map** tab's **UpSet plot, Venn diagram, Karnaugh map**, and **Proportional Venn** now each have an **Add to Report** button in their section heading row, and the four **Column correlation → Paired correlation** cards (**pair scatter, QQ plot, Bland–Altman, correlation with column X**) gained heading rows with the same button. The three linked views and the Proportional Venn register as `svg_host` report sections (`upset_plot` / `upset_venn` / `upset_karnaugh` / `upset_prop_venn`, captured as vector snapshots of the rendered SVG — same pipeline as PCA scree / SAINT); the four paired figures register as `plotly` sections (`corr_pair_scatter` / `corr_pair_qq` / `corr_pair_bland` / `corr_pair_refcorr`) and, like the correlation/distance matrices, the capture prep switches the inner tab to **Paired correlation** before collecting so the embeds reflect the current Column X / Column Y. All eight ids are whitelisted in `validateReportFigurePrerequisites` (figure-must-be-generated-first guard) and got `FIGURE_CAPTION_DEFAULTS` entries; adding the Proportional Venn while it has never been drawn switches to that inner tab first (`switchUpSetInnerTab('propVenn')`) so the capture is populated. ([index.html](index.html), [js/nebula_report.js](js/nebula_report.js), [README.md](README.md), [doc/upsetvenn.md](doc/upsetvenn.md), [doc/columncorrelation.md](doc/columncorrelation.md), [doc/docs_bundle.js](doc/docs_bundle.js))
- **Data QC → Overall — matrix summary moved into the sidebar**: the pill strip (`#dataQcOverallSummaryStrip`, "N features (rows)" / "N samples (columns)") that sat at the top of the figures column moved into the sidebar as a **Matrix summary** section (right under the intro, above Refresh dashboard), freeing the whole top of the figure area for the charts — the sub-tab row is now the first element of the figures column. The redundant third pill ("Per-column bars: Data QC → Overall sidebar") was dropped from `summaryStripHtmlFromStats()` since the strip now sits next to that very control, and the strip CSS (`css/main.css`) was re-tuned for the narrow sidebar (tighter gaps, no outer margin). The empty state ("Load a matrix to see summary statistics.") still shows there when no matrix is loaded. ([index.html](index.html), [css/main.css](css/main.css), [js/data_qc_overall_dashboard.js](js/data_qc_overall_dashboard.js), [doc/overview.md](doc/overview.md), [doc/docs_bundle.js](doc/docs_bundle.js))
- **Column profile — five per-sample figures split into sub-tabs**: the **treemap, ranked intensity (bar), cumulative %, pie, and intensity-distribution (histogram + KDE)** figures no longer stack in one long panel; `#colProfilePlotsWrap` now leads with a flat-underline sub-tab row (`#colProfileInnerSubTabsRow`, `data-col-profile-inner="tree|bar|cum|pie|hist"`) with **Treemap as the first and default tab**. `renderColumnProfilePlots()` now draws **only the active tab's figure** (so a figure is never drawn while hidden — avoiding the Plotly fallback-size lock), `switchColumnProfileInnerTab(name)` / `applyColumnProfileInnerTabLayout(next)` drive the panes (flex/none) and tab buttons, and `resizeColumnProfilePlots()` re-fits the visible figure after every redraw and on sub-tab (re)activation. **The active figure now stretches to the full height of the visible panel**: the tab row moved out of `#colProfilePlotsWrap`, each pane (`#colProfilePlotsInnerTree/Bar/Cum/Pie/Hist`) is a flex column with `min-height: 100%`, and the plot card + host grow (`flex: 1`) instead of the old fixed 340/380px caps (same flex chain as the Column correlation → Distance matrix, v5.38). The report collector switches to the figure's own tab before capture (`top_features_bar` → `bar`, `column_profile_treemap` → `tree`) and the Add to Report button tooltips were updated. ([index.html](index.html), [css/main.css](css/main.css))
- **Column profile — per-column tab row replaced by a sidebar dropdown**: the row of **one button per sample column** above the plots (`#colProfileSampleSubTabs`) is replaced by a **Sample column** select (`#colProfileSampleSelect`) at the top of the **Display options** sidebar section; picking a column routes through the same `switchColumnProfileSample(idx)` → re-render path. `buildColumnProfileSampleTabs()` became `refreshColumnProfileSampleOptions()` + `syncColumnProfileSampleSelect()` (options rebuilt from `currentData.columnHeaders`, index clamped/synced into the select wherever the index is restored), the dead `.col-profile-subtabs-row` / `.col-profile-sub-tab` CSS was removed, and the **Data QC side of the data-change hook** in `updateTablePreview()` now re-activates the Column profile (and Column correlation) sub-tabs too — the old guard only covered Data PreProcess — so loading data while the sub-tab is already open repopulates the dropdown and redraws immediately. ([index.html](index.html), [css/main.css](css/main.css), [doc/columnprofile.md](doc/columnprofile.md), [doc/docs_bundle.js](doc/docs_bundle.js))
- **Column correlation — Color theme selector**: a **Color theme** dropdown (`#colCorrThemeSel`) at the top of the matrices sidebar block — visible on both the **Correlation matrix** and **Distance matrix** tabs — restyles both figures with the same theme families as Row Profile / Proportional Venn: **Default** (RdBu cells + `#8e2a6b` scatter dots + Viridis distance heatmap), **Distinct (Category10)**, **Warm**, **Cool**, **Pastel**, **Dark**, or **High contrast**. The correlation cells use `colCorrThemeScale(false)` (diverging scales: RdBu / RdYlBu / Spectral / BrBG / PuOr / Earth / Picnic), the scatter dots take `getRowProfileThemeColor(theme, 3)`, and the distance heatmap uses `colCorrThemeScale(true)` (sequential scales: Viridis / Plasma / YlOrRd / Blues / Mint / Inferno / Jet). The choice persists in `localStorage` (`nebulaColCorrTheme`) and re-renders live on change. ([index.html](index.html))
- **Column correlation — Overall correlations split into two inner tabs**: the single **Overall correlations** inner tab (which stacked the scatter-correlation matrix and the distance heatmap on one page) is replaced by **Correlation matrix** (`corr`) and **Distance matrix** (`dist`) inner tabs beside **Paired correlation** (`paired`). Each tab renders only its own figure on switch/refresh (so a figure is never drawn while hidden — avoiding the Plotly fallback-size lock), the **Distance from correlation** field (`#colCorrDistanceModeField`) is hidden on the Correlation matrix tab (that tab doesn't use it), and the status line reports per-figure settings (e.g. "Distance heatmap (1−r): 30 column(s)…"). The **distance heatmap now fills the full panel height**: `#colCorrPlotsInnerDist` switches to a flex column (`min-height: 100%`), the plot card and `.col-corr-heatmap-host` grow (`flex: 1`) instead of the old fixed `400px`, and `scheduleColumnCorrelationRender()` / `refreshColumnCorrelationPlots()` re-run `resizeColumnCorrelationPlots()` after every redraw so the figure stays stretched after option changes. The report collector switches to the figure's own tab before capture (`corr_scatter_matrix` → `corr`, `corr_distance_heatmap` → `dist`) and the Add to Report button tooltips were updated; legacy `'matrices'` values map to `'corr'`. ([index.html](index.html), [css/main.css](css/main.css))
- **Proportional Venn — Shape/Color theme + notes moved beside the diagram**: the two selectors were removed from the Proportional Venn heading row (which now holds only the "Proportional Venn" title) and placed in a **210px options column to the right of the diagram** (`#upsetPropVennOptsCol` inside `#upsetPropVennRow`), because the near-square figure leaves wasted horizontal space beside it. The column also hosts the **"Notes & attribution"** details (moved out of the pane's top), pinned to its bottom (`margin-top: auto`); it is only present on the **Proportional Venn** inner tab (the pane itself is hidden on *Linked views* — the old sidebar section and its `switchUpSetInnerTab` visibility toggle were removed). Reuses `.upset-comb-field` full-width select styling; stacks above the diagram under 900px via a media query in `css/main.css`. Selector ids/onchange wiring unchanged, so both `localStorage` persistence and live re-render keep working. ([index.html](index.html), [css/main.css](css/main.css))
- **Proportional Venn — Color theme dropdown**: the **Color theme** selector (next to Shape) choosing the palette for the set shapes and overlap regions — **Default** (classic tableau), **Distinct (Category10)**, **Warm**, **Cool**, **Pastel**, **Dark**, or **High contrast** (the same families as the Row Profile color themes, shared palette definitions). Applies to all render paths (built-in 1–3 set circles, single-set shapes, Eunoia 4+ set fits, and the offline ring fallback) and is persisted in `localStorage` (`nebulaUpsetPropVennTheme`, same pattern as the Shape dropdown). ([index.html](index.html))
- **UpSet / Venn / K-map sidebar — Set chooser list height halved**: the scrollable column-checkbox list (`#upsetSetChooserList`) is now `max-height: 110px` (was 220px), so it occupies half the vertical space — about 5–6 checkboxes visible with scrolling. ([css/main.css](css/main.css))
- **UpSet / Venn / K-map sidebar — help moved to a collapsed box at the bottom**: the introductory paragraph (set definition, linked views, UpSet.js/AGPL note, Pop out / `file://` details) was removed from the top of the sidebar (below the "UpSet / Venn / Karnaugh" heading) and is now a **"Help & about this tab"** `<details>` box — **collapsed by default** — at the **bottom of the sidebar**, below the **Refresh plots** button, styled like the "Set combinations (UpSet plot)" box. The Set chooser card is now the first thing under the heading. ([index.html](index.html))
- **Notes / hints / status text unified to 12px app-wide**: the shared `.small` class (used for notes, hints, captions, and status lines on every tab) had no `font-size` rule after the inline-CSS migration and rendered at the 16px base — visibly bigger than the 12px tab labels (most noticeable in Data QC → UpSet/Venn/K-map → Proportional Venn's "Notes & attribution" block). It now pins to `12px`, so all tabs' small text matches the tab-label and table text size. ([css/main.css](css/main.css))
- **Data QC → UpSet / Venn / K-map → Proportional Venn — Shape dropdown**: the section heading now has a **Shape** selector choosing the shape family for the diagram — **Auto** (built-in exact circles for 1–3 sets; ellipses fitted by Eunoia for 4+, with circles as Eunoia fallback), **Circle**, **Ellipse**, **Square**, or **Rectangle** (the same families offered by the Eunoia web app). With an explicit family, **2–3 sets are also fitted by Eunoia** with that shape (replacing the built-in exact/approximate layout) and **4+ sets** use it as the Eunoia family; with a **single selected set the shape is drawn directly** (no CDN needed). The status line reports the family actually used ("Fitted with Eunoia (square, seed 42): diagError …, stress …" vs "Overlap placed exactly" / "Single circle"), and Auto mode news up the built-in solver whenever the CDN is missing. ([index.html](index.html), [README.md](README.md), [doc/upsetvenn.md](doc/upsetvenn.md), [doc/docs_bundle.js](doc/docs_bundle.js))
- **Data QC → UpSet / Venn / K-map → Proportional Venn — diagram fills the panel + collapsed notes**: the SVG now uses the **full height of the visible pane** (measured from `.upset-viz-scroll` at render time instead of a fixed 34%-of-viewport cap), so both the built-in 1–3 set layout and the Eunoia 4+ set fit render much larger on screen; the offline ring fallback scales up with it. The corner **hint + attribution paragraphs collapsed into a "Notes & attribution" `<details>`/summary** (closed by default) so the diagram gets the whole right-panel height; the Eunoia credit line still appears inside when expanded. ([index.html](index.html))
- **Data QC → UpSet / Venn / K-map → Proportional Venn now fits 4+ sets area-proportionally with Eunoia**: previously 4+ sets drew only sized circles in a ring (no overlap fitting), which gets unreadable as the overlap structure grows. The whole layout is now fitted by the **Eunoia** library ([eunoia.bz](https://eunoia.bz), J. Larsson, [github.com/jolars/eunoia](https://github.com/jolars/eunoia), MIT OR Apache-2.0) — lazily imported from jsDelivr as a single self-contained `web.js` with the WASM inlined (no separate `.wasm` fetch, no build step). Exclusive region sizes are computed from the set memberships; `euler()` (fixed seed, reproducible) returns fitted polygons/shapes which render as self-contained SVG with exact per-region feature counts placed at Eunoia's label anchors, set labels with counts, and fit-quality metrics (diagError / stress) in the status line. Ellipses are tried first (Eunoia authors note they fit 3+ sets noticeably better than circles), circles as fallback shape, and the old sized-circle ring if the CDN is unreachable (with an explanatory note; 1–3 sets always use the built-in solver offline). An attribution credit line with the citation appears whenever Eunoia renders. Citation: Larsson & Gustafsson (2018), *A Case Study in Fitting Area-Proportional Euler Diagrams with Ellipses Using eulerr*, Set Visual. Reasoning 2018, CEUR-2116. ([index.html](index.html), [README.md](README.md), [doc/upsetvenn.md](doc/upsetvenn.md), [doc/docs_bundle.js](doc/docs_bundle.js))
- **Report preview — figures grouped into sub-tabs**: the in-app Report preview no longer stacks every selected section in one long column. When more than one section group is enabled, the preview now shows a **sub-tab rail** (one flat-underline tab per group: Data QC → Overall · Data QC → Column profile · Data QC → Column correlation · Data QC → Row profile · Clustering · Downstream → Differential/SAINT/Enrichr), each switching to that group's panel. Interactive figures (Plotly embeds, PCA/PCoA 3D D3) **mount lazily when their tab is first activated**, so hidden panels don't pay the render cost; the last-visited group is remembered across preview refreshes (checkbox toggles, tab switches). Exported HTML keeps its existing linear layout. ([js/nebula_report.js](js/nebula_report.js), [css/main.css](css/main.css), [doc/report.md](doc/report.md))
- **Report sidebar — primary buttons at the top, no default sections**: the **Save Report** / **Generate Report** buttons moved to the top of the Report sidebar (full-width, same secondary style); the fixed **Summary / Matrix and meta summary** checkbox and its **Actions** hint were removed, and `REPORT_SECTIONS_DEFAULT` in [js/nebula_report.js](js/nebula_report.js) is empty — the report starts with no sections selected until you use **Add to Report**. ([index.html](index.html), [js/nebula_report.js](js/nebula_report.js), [doc/report.md](doc/report.md))
- **Report preview — Row profile and Clustering figures fill the panel**: in the in-app preview, figure rows belonging to the **Data QC → Row profile** and **Clustering** groups stretch to **100% of the right-panel width** (Plotly host sized to the visible panel at mount), and the **heatmap** and **row profile** figures additionally **fill the full preview box height** (measured panel height minus figure chrome; falls back to the locked size when the panel is hidden at mount). The saved/exported HTML keeps each figure's locked pixel size. ([js/nebula_report.js](js/nebula_report.js))

## [5.30] - 2026-07-29

### Added
- **Clustering → K-means sub-tab**: a new exploratory clustering view alongside Heatmap / Clustergrammer / PCA / PCoA / t-SNE, for datasets with **no known sample grouping** — it clusters the **samples (columns)** by their feature profiles to reveal potential groups. Advanced options: **k-means++** (or random) initialization, multiple **restarts (n_init)** keeping the lowest-inertia run, configurable max iterations, and a fixed **random seed** for reproducibility. Number of clusters can be **auto-selected** across a k-range by **silhouette** (best separation) or the **elbow / WCSS knee**, or set to a **fixed k**. Three result views: **Cluster plot** (2D PCA projection colored by cluster, with centroids), **Model selection** (elbow + mean-silhouette curves vs k, with the chosen k marked), and **Assignments** (sortable sample→cluster table with per-sample silhouette and PC coordinates, plus **Export CSV**). Preprocessing mirrors PCA/t-SNE (Log10, column/row z-score) and sampling caps keep large matrices responsive; silhouette is skipped above 2000 samples. Heavy computation runs off the main thread in a dedicated worker. ([index.html](index.html), [js/kmeans_analysis.js](js/kmeans_analysis.js), [js/kmeans.worker.js](js/kmeans.worker.js))
- **Labeled "Pop out" button above every figure**: a visible, right-aligned **Pop out** button now sits in a heading row directly above each figure across **Data QC** (Row Profile, Column profile, Column correlation), **Clustering** (PCA 2D/3D + scree, PCoA 2D/3D, t-SNE, K-means cluster + elbow + silhouette), and **Downstream** (Differential volcano / MA / p-value histogram / group heatmap; SAINT scatter + network; Enrichr bar / bubble / term-similarity + results heatmap). Previously most of these offered pop-out only via the small Plotly modebar icon (or a sidebar/in-plot toolbar). Buttons are injected consistently by a new config-driven module and reuse the existing `openPlotlyPopout` / `openSvgPopout` / `openImagePopout` / `openAutoPopout` helpers, so pop-out behavior is unchanged — only discoverability improves. ([index.html](index.html), [js/figure_popout_buttons.js](js/figure_popout_buttons.js))
- **"Add to Report" on primary analysis figures**: the primary output figures that lacked it now have an **Add to Report** button (next to Pop out): **Differential** volcano / MA / group heatmap, **t-SNE**, **K-means** cluster plot, **PCA scree**, **SAINT** scatter + network, **Enrichr** bar / bubble / results heatmap, and **Row Profile**. Each is registered as a report section in `NebulaReport.REPORT_SECTIONS` (grouped under Data QC → Row profile, Clustering, and Downstream → Differential/SAINT/Enrichr), whitelisted in `validateReportFigurePrerequisites` with a "figure must be generated first" guard, and captured via the same generic Plotly-embed / SVG-host pipeline as the existing report figures. The buttons are injected by the same module as the Pop out buttons. ([index.html](index.html), [js/nebula_report.js](js/nebula_report.js), [js/figure_popout_buttons.js](js/figure_popout_buttons.js))

### Removed
- **Dead `popout/heatmap_popout.html`**: deleted the orphaned heatmap pop-out page. Since the pop-out system was unified, the Heatmap tab's **Pop out** routes through the shared `popout/plot_popout.html` (`heatmapOpenPopout()` → `openPlotlyPopout('heatmapPlot', …)`), so nothing loaded `heatmap_popout.html`. Updated the now-stale doc references (UpSet-tab note in [index.html](index.html) and [doc/overview.md](doc/overview.md)) to point at `plot_popout.html` and rebuilt [doc/docs_bundle.js](doc/docs_bundle.js). The `popout/` folder now holds only the four in-use pages (plot / svg / image / upset). ([popout/](popout/), [index.html](index.html), [doc/overview.md](doc/overview.md))

### Changed
- **Downstream buttons on-theme + redundant title removed**: retired the leftover old-theme colors on the Downstream run buttons — **Run differential analysis** (`.diff-run-btn`, was bright green), **Run SAINT analysis** / **Construct network** (were indigo/purple `#5a67d8` / `#764ba2`), and **Run enrichment** (was bright green `#48bb78`) — all now use the themed olive `--md-btn-success`, matching the other Generate/Run CTAs. Also removed the redundant **"Differential analysis"** sidebar heading in the Differential sub-tab (the sub-tab bar already labels it) and its now-unused CSS. ([index.html](index.html), [css/main.css](css/main.css))
- **Heatmap defaults**: the Clustering → Heatmap color scale now defaults to **Viridis** (was RdBlkGn), and **Cluster samples (columns)** is now **unchecked** by default (Cluster features/rows stays checked). ([index.html](index.html))
- **Heatmap Pop out moved into the figure header**: the Heatmap tab's **Pop out** button was moved from the sidebar (next to Generate heatmap) into the plot's heading row, right beside **Add to Report** — consistent with every other figure. It's now injected by `js/figure_popout_buttons.js` (routing through the shared `openPlotlyPopout`), and the bespoke sidebar button + its `heatmapOpenPopout()` helper were removed. ([index.html](index.html), [js/figure_popout_buttons.js](js/figure_popout_buttons.js))
- **Button hierarchy — ghost/outline secondary actions + themed "run" color**: reworked button visuals for a clearer, more on-theme hierarchy (chosen "Option C"). Secondary/utility actions (`.example-btn` — Add to Report, Pop out, Export/Download CSV, example-data loaders, session import/export, Refresh, Clear, …) changed from a gold fill to a **ghost/outline** style (transparent, warm-accent border + text, soft-accent hover), sitting a clear step below the solid **primary** CTAs. The bright green **Generate/Run** buttons (`#4CAF50`) were retired app-wide for the theme's olive `--md-btn-success` (`#6f7f66`), which also re-themes the two matching progress-bar fills. ([index.html](index.html), [css/main.css](css/main.css))
- **Unified header icon buttons into one shared style**: the three header controls — coffee (donate), **?** (docs), and the **+/−** analysis-status toggle — now share a single `.header-icon-btn` class (one 26px round outlined button, accent-fill on hover/active) instead of three near-duplicate rules. The coffee button drops its orange fill to match the others. Less CSS, one consistent look. ([index.html](index.html), [css/main.css](css/main.css))
- **Analysis-status toggle is now a +/- icon button**: the wide "Show more analysis status" / "Collapse analysis status panel" text button became a compact round **+** (collapsed) / **−** (expanded) icon button, matching the header's "?" button, to save space. `aria-label`, `title`, and `aria-expanded` update with state for accessibility. ([index.html](index.html), [css/main.css](css/main.css))
- **"Buy me a coffee" button is now icon-only**: the text label was removed, leaving just the compact coffee icon (a small round button) in the header to save horizontal space. The PayPal link, hover tooltip, and an `aria-label` (accessible name) are unchanged. ([index.html](index.html), [css/main.css](css/main.css))
- **Top tab bar restyled (flat "underline" tabs)**: the main tabs no longer use the bold, UPPERCASE, skewed "chrome" blocks (which read as chunky). They are now flat, slim (font-weight 500; active 600), **sentence-case** labels ("Data preparation", "Data preprocess"; "QC" kept as an acronym), with the active stage marked by a warm accent **underline** instead of a filled gradient block — a lighter, more modern look that fits the flat `--md-*` shell. ([index.html](index.html), [css/main.css](css/main.css))
- **All sub-tab levels aligned to the same flat "underline" language**: every lower tab level — L2 sub-tabs (Data preprocess / Data QC / Clustering / Downstream), the L3 sub-sub-tabs (Differential Volcano/MA/…, SAINT, Enrichr, Column-profile samples, PCA/PCoA/K-means viz tabs, Data Filter Row/Column) and the L4 Enrichr plot-type tabs — previously rendered as skewed tan "wedge" tabs (with a competing dead indigo layer underneath). They now use the identical flat underline treatment as the top bar (transparent, slim weight, accent underline on active), so all tab levels feel like one family. Hierarchy is conveyed by nesting + size rather than by different shapes/colors. The obsolete `--subtab-*`, `--thirdtab-*`, `--fourthtab-*`, and `--angled-tab-*` palettes and the skew/`::before`/`::after` geometry were removed. ([css/main.css](css/main.css))
- **Document tab → header "?" button**: the **Document** main tab was removed from the top tab bar and replaced by a compact circular **?** button in the header, placed right after the version number. This trims the main tab bar to the six analysis stages (Data preparation · Data QC · Data preprocess · Clustering · Downstream · Report). The button calls `switchTab('document')` (unchanged panel/behavior) and highlights while the Document panel is open. ([index.html](index.html), [css/main.css](css/main.css))
- **Unified table font size (12px)**: every table rendered through the shared `TableDisplay` module (Data Filter matrix preview, Differential results, Enrichr, SAINT, DIANN annotations, Name mapping, static previews) now uses a consistent **12px** typography via the shared `qc-dt-preview-host` container, matching the Meta table and matrix preview. This reduces the previously oversized **Downstream → Differential → Results table** to a more professional size. ([css/main.css](css/main.css))

### Fixed
- **Figure heading-row button alignment**: in the Data QC → Overall cards the **Pop out** button was stranded in the center of the heading row (the card title lacked `flex-grow`, so `justify-content: space-between` spread the buttons apart). The title now flex-grows so the **Pop out** and **Add to Report** buttons group cleanly at the right edge. ([css/main.css](css/main.css))
- **Equal button heights**: the **Pop out** button (which carries an inline SVG icon) rendered slightly taller than the text-only **Add to Report** button. Both heading-row buttons now share one box model (inline-flex, matching font/padding/line-height), so they align at the same height. ([css/main.css](css/main.css))

## [5.29] - 2026-07-17

### Added
- **Pop out on (almost) every figure**: the full-size "Pop out" workspace — previously only on the Heatmap and UpSet/Venn/K-map charts — is now available across the app. Rather than one bespoke page per figure, there are three generic receiver pages by rendering technology, plus shared helpers (`openPlotlyPopout` / `openSvgPopout` / `openImagePopout` / `openAutoPopout`):
  - `popout/plot_popout.html` — **all Plotly figures**. A "Pop out" button is auto-added to every Plotly modebar (by wrapping `Plotly.newPlot`/`react` once), so volcano, MA, p-value histogram, differential group-heatmap, column-profile bar/treemap, column-correlation scatter-matrix/distance-heatmap, row-profile, Enrichr heatmap, t-SNE, the clustering heatmap and the row-filter summaries all gain it with no per-figure wiring. The popped-out plot keeps full Plotly interactivity (zoom/pan/hover/export).
  - `popout/svg_popout.html` — **D3/SVG figures** via `<svg>` cloning: the three Data QC → Overall charts (per-column summary, box plots, total-log) and **Clustergrammer**. Static but vector and browser-zoomable.
  - `popout/image_popout.html` — **canvas/WebGL figures** via a static PNG snapshot: **PCA/PCoA 3D**. PCA/PCoA 2D (SVG) and 3D (canvas) both get a Pop out button on their existing export toolbar, auto-routed to the SVG or image page as appropriate.
  - The window handoff reuses the proven pattern (sessionStorage + retried `postMessage`) so it also works under `file://`. The heatmap's own Pop out now routes through the shared Plotly helper (its dedicated `heatmap_popout.html` is retained for backward compatibility). ([index.html](index.html), [popout/plot_popout.html](popout/plot_popout.html), [popout/svg_popout.html](popout/svg_popout.html), [popout/image_popout.html](popout/image_popout.html))

## [5.28] - 2026-07-17

### Changed
- **Top-level tabs — grouped downstream analyses**: the three separate top-level tabs **Differential**, **SAINT**, and **Enrichr** are now consolidated into a single **Downstream** top-level tab, where they appear as second-level sub-tabs (Differential | SAINT | Enrichr). This declutters the header and leaves room for more downstream analyses to be added under the same tab. Each panel keeps its own id/behaviour (including its own third-level sub-tabs, e.g. SAINT's Network/Scatter and Enrichr's Enrichment plots/Heatmap); a redirect keeps any existing `switchTab('diff'|'saint'|'enrichr')` links working by opening the matching sub-tab. The active-tab highlight was also made order-independent (matched by each button's `switchTab` target), fixing a latent mismatch that highlighted the wrong button between Data QC and Data PreProcess ([index.html](index.html)).

## [5.27] - 2026-07-17

### Fixed
- **Data QC → Row Profile (stacked % composition)**: the plot could badly misrepresent which rows dominate. With more than 30 rows selected (e.g. **"Select all Cont"**, which selects all contaminants in file/alphabetical order), the chart drew only the **first 30 by selection order** and, worse, computed the **% composition denominator over just those 30 shown rows**. So the biggest contributors — which happened to sort later alphabetically — were dropped entirely, and the remaining segments were inflated to fill 100%. In a total-proteome dataset this made **trypsin (`Cont_P00761`)** appear to be ~80% of the contaminant signal when it is actually ~10%; the true top contaminants, **Fetuin-A (`Cont_P12763`, ~31%)** and **BSA (`Cont_P02769`, ~27%)**, were not plotted at all. Two changes fix it: (1) when the selection exceeds the 30-trace cap, rows are now **ranked by total intensity** before truncation, so the largest contributors are the ones shown (title now reads "top 30 of N by intensity"); (2) the **% composition denominator now spans every selected row**, with a gray **"Other (N more rows)"** band for the untruncated remainder, so each segment reflects its true share and the stack still reaches 100% ([index.html](index.html)).

### Changed
- **Data QC → Row Profile**: the default **Color theme** is now **Pastel** (was Default/Plotly), which reads better alongside the red contaminant-mark outline on the bars ([index.html](index.html)).

## [5.26] - 2026-07-16

### Added
- **Contaminant proteins — detection & marking**: protein groups whose accession starts with **`Cont_`** (or MaxQuant's `CON__`) are now flagged as contaminants (checked against the row id and the protein/accession annotation columns). A new option **"Mark contaminant proteins (Cont_)"** (default **checked**) appears in both the **Data QC → Row Profile** and **Column profile** sub-tabs and is kept in sync between them:
  - *Row Profile*: contaminant rows get a red **⚠Cont** badge in the selection checklist, and contaminant traces in the bar/line/stacked plot are marked (name suffixed `⚠(Cont)`, red bar outline / dotted line). A **"Select all Cont"** quick-selection button (below "Select top N") selects every contaminant protein group in one click, ready to plot.
  - *Column profile*: contaminant features are drawn **red** in the ranked bar chart and outlined red in the treemap/pie, and the status line reports **how much of the sample's total intensity comes from contaminants** (e.g. "⚠ Contaminants: 3 features, 62.50% of total intensity in this sample") — making per-sample contamination easy to compare. Unchecking the option removes all marking ([index.html](index.html)).

### Changed
- **Data Preparation → Upload data file**: the default file format is now **DIANN protein groups** (`results.pg*_matrix.tsv`) instead of Plain / clean data matrix — matching the most common upload. The matching options panel and the file picker's accepted extensions are pre-set on load to stay consistent with the default ([index.html](index.html)).

### Added
- **Auto meta from sample names — time-course detection**: the meta-guessing parser now recognizes a **time axis** in structured sample names and gives it its own `Time` column instead of folding it into the group. It scans the `_`/`-`/`.`-separated token positions and picks the one where most values look like a duration (number + unit: `s`, `min`, `h`, `d`, `w`, `month`, …); non-duration labels sharing that position (e.g. `NS` for an unstimulated/baseline point) are kept as-is. The time token is then removed from the `Group` label. Example: `S01_D292_NS`, `S02_D292_5min`, `S03_D292_15min`, … now yield **Sample** = `S01…S14`, **Group** = `D292/D327/…`, **Time** = `NS/5min/15min/30min` (previously Group was `D292_NS`, `D292_5min`, …). Detection fires only when the structure is consistent and the axis actually varies, so datasets without a time-course are unaffected ([index.html](index.html)).
- **Auto meta — S/R/T labels keep zero-padding**: the `Sample` (S#), `Biological_Replicate` (R#), and `Technical_Replicate` (T#) columns now preserve the original digit string from the sample name (e.g. `S01`, not `S1`). This keeps labels aligned and string-sortable in numeric order (`S01..S14` instead of `S1, S10, S11, …, S2`). The 1..N completeness check still uses the parsed number, so detection is unchanged ([index.html](index.html)).

### Fixed
- **Clustering → PCoA / all scatter labels**: sample-name labels in the **PCoA 2D** plot were placed far from their points with long, crossing leader lines (much worse than PCA). Root cause: the label-placement optimizer's Adam step size (`lr`), `smoothEps`, and the finite-difference floor were absolute *data-unit* constants tuned for PCA-scale coordinates (~10-100). PCoA coordinates from non-Euclidean metrics (e.g. Bray-Curtis) span ~0.1-1, so a single step (`lr = 0.32`) exceeded the whole plot and labels slammed into the axis bounds. `optimizeScatterLabels` now **normalizes coordinates to a canonical span before optimizing and maps results back**, making label placement scale-invariant. PCoA leader lines dropped from mean ≈131 px / max ≈295 px to ≈58 px / ≈92 px, matching PCA; PCA and t-SNE placement is unchanged ([js/scatter_label_optimize.js](js/scatter_label_optimize.js)).

### Changed
- **Clustering → PCA / PCoA (internal)**: the near-identical `drawPca2dD3` and `drawPcoa2dD3` renderers (~180 lines each) are unified into a single `drawScatter2dD3(d3mod, model, opts, cfg)`; `drawPca2dD3`/`drawPcoa2dD3` are now thin wrappers that only supply the differing DOM ids, axis/title text, and state bags via `cfg`. No behavior change (titles, axis labels, ellipses, legend hide/show, zoom-reset, export buttons, and the report/details visible-domain cache all verified identical); removes ~110 lines and keeps the two plots from drifting apart ([index.html](index.html)).

### Performance
- **Scatter label placement**: `optimizeScatterLabels` now uses an **analytic energy gradient** (single O(n²) pass) instead of finite differences (2·2n energy evaluations per step, each O(n²) → O(n³) per step). The gradient is verified against finite differences (max relative error ~1e-6). Placement output is unchanged (≤1.2% of plot span). Measured speedups on the label optimizer: **n=40 ~44×**, **n=90 ~180×** (2.7 s → 15 ms), **n=150 ~385×** (11.3 s → 29 ms) — the label cap (150) no longer freezes the UI on label-dense plots ([js/scatter_label_optimize.js](js/scatter_label_optimize.js)).

### Fixed
- **Asset caching**: local `css`/`js` includes now carry a `?v=5.25` cache-busting query (previously only one file did, pinned at a stale `?v=5.11`). Browsers were serving outdated cached JS/CSS after an app update; the version query is documented to bump with each release ([index.html](index.html)).

## [5.24] - 2026-06-30 16:12:23

### Fixed
- **Data Preparation → Try example data**: **MaxQuant proteinGroups** and **MaxQuant Phospho (STY)Sites** now load from embedded JS in `js/` (lazy TSV getter, same as DIANN pg matrix) instead of `fetch()` on root `.txt` files — fixes example buttons when the app is opened from `file://` or when the TSV is not served alongside `index.html` ([index.html](index.html), [js/example_maxquant_protein_groups.js](js/example_maxquant_protein_groups.js), [js/example_maxquant_phospho_sites.js](js/example_maxquant_phospho_sites.js), [scripts/build_maxquant_examples.py](scripts/build_maxquant_examples.py)).

## [5.23] - 2026-06-30 15:30:04

### Added
- **Data Preparation → Data matrix**: the file uploader is now **format-first** — pick the format from a dropdown (Plain / DIANN pg / DIANN gg / **MaxQuant proteinGroups.txt** / **MaxQuant Phospho (STY)Sites.txt**), then choose the file. Each format shows its own options, and a single file input routes to the matching parser ([index.html](index.html)).
- **MaxQuant support**: built-in parsers for `proteinGroups.txt` and `Phospho (STY)Sites.txt`. They smartly separate the **per-sample intensity matrix** from descriptive **annotation** columns and guess meta info from the experiment names:
  - *proteinGroups*: choose **LFQ intensity** (default when present) or raw **Intensity**; row ID from **Gene names** (default) / Protein names / Protein IDs / Majority protein IDs / id.
  - *Phospho (STY)Sites*: per-sample **Intensity** (multiplicity-summed) columns; row ID as **Gene + site** (e.g. `AKT1_S473`, default) / Protein + site / Leading proteins + position / id.
  - Optional **Remove Reverse & contaminants** (and *Only identified by site* for proteinGroups), on by default. Descriptive columns populate the **Annotations** sub-tab, tooltips, and feature-label dropdowns (same plumbing as DIANN) ([index.html](index.html)).
- **Try example data**: new **MaxQuant proteinGroups** and **MaxQuant Phospho (STY)Sites** buttons load embedded JS (`js/example_maxquant_protein_groups.js`, `js/example_maxquant_phospho_sites.js`) with lazy TSV getters — same pattern as DIANN; works offline without fetching root `.txt` files ([index.html](index.html), [scripts/build_maxquant_examples.py](scripts/build_maxquant_examples.py)).
